...
public class HighRiseBuilding {
    ...
    private synchronized void action(Elevator elevator, Elevator elevatorCompare){
        ...
        // если лифт может быстрее добраться до пассажиров, при условии, что они есть, можно изменить состояние
        // есть 2 варианта, лифт стоял или двигался
        if (!passengers.isEmpty()) {
            startElevator(elevator);
        }

        if (elevator.getOrientation() == -1 && positionElevator - 1 >= 1) {
            elevator.down();
        } else if (elevator.getOrientation() == 1 && positionElevator + 1 <= countFloor) {
            elevator.up();
        }
    }
